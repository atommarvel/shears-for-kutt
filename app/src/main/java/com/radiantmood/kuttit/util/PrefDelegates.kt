...
inline fun <reified T> getPrefDelegate(prefs: SharedPreferences, key: String, default: T?): PrefDelegate<T> {
...
class BooleanPref(private val prefs: SharedPreferences, private val key: String, private val default: Boolean?) :
    Pref<Boolean> {
    override fun get(): Boolean? = if (prefs.contains(key)) prefs.getBoolean(key, false) else default
    override fun set(value: Boolean) = prefs.edit { putBoolean(key, value) }
}

class LongPref(private val prefs: SharedPreferences, private val key: String, private val default: Long?) : Pref<Long> {
